...
Companion to `README_hard_negative_finetune.md`. The two runbooks are not
interchangeable; read the next section before doing anything.
...
| What adapts | classifier heads (backbone freezable) | **features** (the backbone must adapt) |
...
teaches the model to suppress them, which makes recall worse. Recall requires
...
`palm_inference_pipeline.py` ships `SCORE_THR = 0.35`,
...
`test_cfg.rcnn.max_per_img = 300` per 1024 px tile. At
...
model.test_cfg.rcnn.max_per_img=600`). No retraining is needed.
...
The codec must match. `COCO/GE_15cm/train_GE/JPEGImages` is JPEG. Writing new
...
If the AOIs are palm-free areas (for example a dedicated palm-free extent
...
`--aoi` takes either one shapefile for everything, or a folder of
...
Vet the coverage before labelling. Open `tile_footprints.gpkg` over the
imagery in QGIS. Confirm the tiles land where intended and that the AOIs of
interest actually produced tiles. Fixing coverage now costs minutes; after
labelling it costs days.
...
`HP_WEIGHT` is not a share but a multiplier. `SensorBalancedSamplerN`
