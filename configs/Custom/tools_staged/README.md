...
Supports `4_satellite_wv3_30cm/`. Read `STAGE_D_README.md` first.

| script | what it does |
|---|---|
| `build_budget_manifests.py` | builds the **nested, seeded** annotation-budget subsets. Run once, then freeze — rebuilding changes which tiles are in each budget and turns a measurement into an artefact |
...
```
build_budget_manifests.py     # once, then never again
