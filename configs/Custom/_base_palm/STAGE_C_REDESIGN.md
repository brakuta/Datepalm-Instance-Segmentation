...
> This document is the historical engineering record of the
...
This document analyses the reported Stage C failure modes (severe
...
WSL2/NVIDIA driver relocates device allocations into shared (system) memory
rather than raising an out-of-memory error.
...
This was resolved by examining the Stage B per-backbone configs. None
...
comparison the benchmark is designed to support. These divergences, none of
which raises a warning at run time, are summarised below.
...
| `auto_scale_lr` | disabled | enabled, base 16 | Rescales the configured LR by 0.125 with no message in the log |
...
2. Malformed `env_cfg` override (no error raised). `maskrcnn_mambavision_s_stagec.py`
...
5. `auto_scale_lr` rescaling (no error raised). Enabled with `base_batch_size=16`
...
7. AMP `batched_nms` dtype mismatch (run-blocking under `amp_fp16`). Under
   ...
   backbones under AMP to completion, which indicates the mismatch is build-
   and path-dependent rather than intrinsic to AMP. The fix is a minimal,
...
Defence in depth is a historical aside that applies to the original
workstation only. The training hosts were Windows/WSL2 workstations, where
the NVIDIA driver spills device allocations into shared system memory instead
of raising an out-of-memory error. On that environment, the NVIDIA Control
Panel option *Manage 3D Settings → CUDA — Sysmem Fallback Policy → Prefer No
Sysmem Fallback* converts any residual overflow into a fast, explicit
out-of-memory error rather than a host-freezing spill that produces no error
at all. With the working set inside 24 GB this path is never exercised, and
the setting is irrelevant on native Linux hosts; it is recorded here as part
