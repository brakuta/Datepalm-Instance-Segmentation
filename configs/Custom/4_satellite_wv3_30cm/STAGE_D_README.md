...
**63,946 distinct reference crowns.** Quote that, not the 178k polygons — 50%
overlap writes each training crown up to four times.
...
the pixels, so it no longer applies — but confirm rather than assume, because
...
`s` and `bu` are refused rather than silently running a cut arm.
...
* the dataset line must read **3,636** train images. Lower means
  ...
  background-supervision decision was silently reverted.
