...
# including the Stage C anchor override -- and only the schedule base and
# frozen_stages differ. See the ConvNeXt-T arm-B0 config for the rationale.
#
# NOTE ON `pretrained`: MM_SpatialMamba loads its ImageNet weights through the
# backbone's own `pretrained` path, not through init_cfg. That file must exist
# for arm B0 to mean anything -- if it is missing, the backbone starts from
# random init and the arm silently becomes "from scratch". Verify:
#   ls -la /workspace/mmdetection/checkpoints/spatialmamba/spatialmamba_small_in1k.pth
# and check the log says "Successfully load ckpt", not "Failed loading".
# (For arm C this does not matter: load_from overwrites the whole backbone.)
# ==========================================================================
...
work_dir = '/workspace/mmdetection/work_dirs/Stage_D/maskrcnn_spatialmamba_s_staged_full'
