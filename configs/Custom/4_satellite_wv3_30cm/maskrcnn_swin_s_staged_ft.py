# ==========================================================================
# maskrcnn_palm_staged/maskrcnn_swin_s_staged_ft.py   (Stage D, v1)
# --------------------------------------------------------------------------
...
# Portable. Swin native to MMDet -> runtime imports/hooks inherited.
# Relative checkpoint path: ls checkpoints/swin_small_p4_w7_backbone_only.pth
...
model = dict(
    backbone=dict(
        ...
        init_cfg=dict(
            type='Pretrained',
            # swin_small_p4_w7_mmdet.pth, not ..._backbone_only.pth.
            # mmdet's SwinTransformer.init_weights keeps only keys prefixed
            # 'backbone.' and then indexes [0] on the result. The
            # _backbone_only file holds 357 BARE keys (patch_embed.*,
            # stages.*), so the filtered dict is empty and init_weights dies
            # with IndexError: list index out of range -- before a single
            # iteration, and with nothing in the message about prefixes.
            # The _mmdet file is the same 357 tensors with the prefix added.
            # Both arms need this: load_from overwrites the backbone for
            # arm C, but init_weights still runs first and still crashes.
            checkpoint='checkpoints/swin_small_p4_w7_mmdet.pth',
        ),
    ),
    neck=dict(
        _delete_=True,
...
work_dir = '/workspace/mmdetection/work_dirs/Stage_D/maskrcnn_swin_s_staged_ft'
...
# Explicit custom_hooks. Without this block Swin-S inherits the base list,
# which carries EarlyStopping patience 4 -- half the 8 the other three arm-C
# configs use -- so this backbone would have been stopped twice as eagerly as
# its competitors, and any shortfall in its row would have been a stopping-rule
# artefact. The base also profiles FLOPs at 1024 px, four times the 512 px
# Stage D actually runs.
custom_hooks = [
    dict(
